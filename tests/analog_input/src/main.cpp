...
//
// The handle is filled by xTaskCreate() when AnalogInputTask is started. It
...
void AnalogInputTask(void *pvParameters);
...
void StartAnalogInputTasks(void)
{
	BaseType_t result = pdFAIL;
	uint8_t priority = 0;
	
	result = xTaskCreate(AnalogInputTask, "tsk_AnalogInput", (configMINIMAL_STACK_SIZE) * 2, NULL, priority, &handle_UserTask);
	if (result == pdPASS )
	{
		// The task was created successfully. The scheduler will call AnalogInputTask
		// when the task receives processor time.
	}
	// When creation fails, the task handle remains NULL. There is currently no
	// recovery action here, so the application continues without AnalogInputTask.
}
...
void AnalogInputTask(void *pvParameters)
{
	...
	ts_printf("> AnalogInputTask started\n");
	...
	while (true)
	{
		adc_voltage = adc.ReadVoltage(channelNr);
		ts_printf("> AnalogInputTask read ADC channel %d: %f\n", channelNr, adc_voltage);
#if 1
		channelNr++;
		if (channelNr >= N_ADC_CHANNELS)
		{
			channelNr = 0;
		}
#endif
...
void setup (void)
{
	...
	// observed independently from the LEDs controlled by AnalogInputTask.
